...
// internally.
//-----------------------------------------------------------------------------
void wave_player::play(FILE *wavefile, volatile bool *play_ptr, volatile int *volume_ptr)
{
  ...
  while (!feof(wavefile) && *play_ptr == true) {
    if (verbosity)
...
// with DC offset so it can be written to the DAC.
         ...
         // Scale data according to volume
           dac_data = (dac_data * (16 - *volume_ptr)) / 16;
